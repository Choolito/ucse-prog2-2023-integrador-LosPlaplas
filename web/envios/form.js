...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (guardarParada) {
    guardarParada.addEventListener("click", (event) => {
...
function generarParada(idEnvio) {
  if (confirm("¿Está seguro que desea generar esta parada?")) {
    ...
    const data = {
      ...
      kmRecorridos: Number.parseInt(kmRecorridos, 10), // Asegurar que es un número
    }
...
function finalizarEnvio(idEnvio) {
  if (confirm("¿Está seguro que desea finalizar el envío?")) {
    ...
    const data = {
      ...
      kmRecorridos: Number.parseInt(kmRecorridos, 10),
    }
...
function exitoGenerarParada(response) {
  ...
  window.location.href = "/web/envios/index_envio.html" // Redirección absoluta
}
